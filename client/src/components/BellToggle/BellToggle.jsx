...
const WARP = 0.6;
const SIZES = { sm: [36, 12.5, 16, 14, 8], md: [40, 13.5, 18, 16, 8], lg: [48, 15, 20, 20, 10] };
...
export default function BellToggle({
  ...
  }, [offLabel, onLabel, size, iconOnly]);
  ...
  useEffect(() => {
    ...
    if (count > was && !reduce) swing(ringAmplitude * WOBBLE.amplitude, WOBBLE.passes, WOBBLE.duration);
  }, [count]);
  ...
  return (
    <motion.span
      ref={rootRef}
      className={`bell-toggle ${iconOnly ? 'bell-toggle--icon-only' : ''}${className ? ` ${className}` : ''}`}
      data-on={on ? 'true' : 'false'}
      data-disabled={disabled ? '' : undefined}
      style={{
        '--bt-clip': iconOnly ? '0px' : clip,
        '--bt-color': color,
        '--bt-bg': background,
        '--bt-on-color': onColor,
