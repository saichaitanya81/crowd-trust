  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center space-y-3">
        ...
        <p className="text-xs text-slate-500">Loading creator operations center...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Campaign Creator Operations Hub</p>
          </div>
        </div>

        <Link
          to="/creator/campaigns/new"
          className="btn-primary text-xs py-3 px-4 gap-1.5 self-start sm:self-auto shadow-sm"
        >
            ...
            <div className="grid grid-cols-1 gap-4">
              {campaigns.map((camp) => {
                ...
                return (
                  <div
                    key={camp._id}
                    className="card-container p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-brand-300 transition-all"
                  >
                    <div className="flex items-start gap-4 min-w-0">
                      ...
                      <div className="min-w-0 space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="badge bg-slate-100 text-slate-700">{camp.category}</span>
                          <span
                            className={`badge ${
                              camp.status === 'active'
                        ...
                        <h3 className="text-base font-bold text-slate-900 truncate">{camp.title}</h3>
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 shrink-0">
                      ...
                      <div className="w-44 space-y-1">
                        <div className="flex justify-between text-xs font-bold">
                          <span>₹{Number(camp.raisedAmount).toLocaleString()}</span>
        ...
        <div className="max-w-2xl">
          <div className="card-container p-6 sm:p-8 space-y-6">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
              <div>
                <span className="text-xs text-slate-500 block">Current Verification Status:</span>
                <span className="text-sm font-extrabold uppercase mt-0.5 block text-slate-900">
